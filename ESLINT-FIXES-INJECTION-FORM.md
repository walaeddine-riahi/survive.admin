...
resolver: zodResolver(injectionFormSchema) as any,
```
*Note: Commentaire ESLint ajouté car le `as any` est nécessaire ici pour la compatibilité des types.*
...
<Controller
  ...
  render={({ field }) => (
```
*Note: Commentaire ESLint ajouté car le `as any` est nécessaire pour le Controller.*
